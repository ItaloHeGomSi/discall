  ...
  const startRecording = async (streamToRecord?: MediaStream | null) => {
    try {
      ...
      recorder.onstop = async () => {
        ...
        const fileName = `discall_clip_${timestamp}.webm`;

        try {
          // Tentativa de escrita nativa via Tauri Plugin FS v2
          const { writeFile, mkdir, BaseDirectory } = await import('@tauri-apps/plugin-fs');
          await mkdir('Discall/Clips', { baseDir: BaseDirectory.Video, recursive: true });
          await writeFile(`Discall/Clips/${fileName}`, uint8Array, {
            baseDir: BaseDirectory.Video,
          });
          console.log(`[Discall Clips] Arquivo salvo em disco: ${fileName}`);
        } catch (tauriErr) {
          console.warn('[Discall Clips] FS Tauri não disponível, salvando em memória web:', tauriErr);
        }

        // Adiciona à store de clipes
        addClip({
          id: `clip-${Date.now()}`,
          fileName,
          filePath: `Discall/Clips/${fileName}`,
      ...
      console.error('[Discall Clips] Erro ao iniciar gravação:', err);
      setIsRecording(false);
    }
  };
